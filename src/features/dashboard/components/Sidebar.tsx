...
export const Sidebar: React.FC<SidebarProps> = ({ user, onClose }) => {
  ...
  const tuteeNavItems = [
    { path: isTutor ? '/dashboard/learn/discover' : '/dashboard/discover', icon: LayoutDashboard, label: 'Discover' },
    { path: isTutor ? '/dashboard/learn/sessions' : '/dashboard/sessions', icon: Calendar, label: 'My Sessions' },
    { path: isTutor ? '/dashboard/learn/history' : '/dashboard/history', icon: Clock, label: 'History' }
  ];
  const navItems = isTutor && !isLearningMode ? tutorNavItems : tuteeNavItems;

  const handleBecomeTutor = async () => {
    const confirmed = window.confirm('Create your tutor profile and switch to the tutor dashboard?');
    if (!confirmed) return;
  ...
  return (
    <div className="flex flex-col h-full">
        ...
        {isTutor && (
          <div className="grid grid-cols-2 gap-2 mb-4 px-2">
            <NavLink
              to="/dashboard"
              onClick={onClose}
              className={`text-center px-3 py-2 rounded-lg text-sm font-medium ${!isLearningMode ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-700'}`}
            >
              Tutor
            </NavLink>
            <NavLink
      ...
      <div className="p-4 border-t border-gray-100">
        ...
        <button className="flex items-center gap-3 px-4 py-3 w-full rounded-xl text-red-600 hover:bg-red-50 font-medium transition-colors">
